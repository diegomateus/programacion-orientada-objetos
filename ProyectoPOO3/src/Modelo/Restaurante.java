...
public class Restaurante implements Serializable,IRestaurante {//Atributo que define que la clase es serializable.
	...
	public List<EnumDia> menuDia(){
		...
		List<EnumDia>dias = new ArrayList<EnumDia>();
	...
	public void agregarPlatoCarta(int c, String n, String dia) {
		...
		if(dia.equals("lu")) {
			...
			temp = EnumDia.DOMINGO;
		}

		Plato plat = new PlatoCarta(c, n, 0, temp);
		platos.add(plat);
	}
	/**
	...
	public boolean verificarDia(int cod) {
		...
		if(numeroDia == 1) {
			palabra = EnumDia.LUNES;
		}

		if(numeroDia == 2) {
			palabra = EnumDia.MARTES;
		}

		if(numeroDia == 3) {
			palabra = EnumDia.MIERCOLES;
		}

		if(numeroDia == 4) {
			palabra = EnumDia.JUEVES;
		}

		if(numeroDia == 5) {
			palabra = EnumDia.VIERNES;
		}

		if(numeroDia == 6) {
			palabra = EnumDia.SABADO;
		}

		if(numeroDia == 7) {
			palabra = EnumDia.DOMINGO;
		}
